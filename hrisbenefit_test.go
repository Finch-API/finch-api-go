...
func TestHRISBenefitNewWithOptionalParams(t *testing.T) {
	...
	_, err := c.HRIS.Benefits.New(context.TODO(), finchgo.HRISBenefitNewParams{Type: finchgo.F(finchgo.BenefitType401k), Description: finchgo.F("string"), Frequency: finchgo.F(finchgo.BenefitFrequencyOneTime)})
	if err != nil {
...
func TestHRISBenefitUpdateWithOptionalParams(t *testing.T) {
	...
	_, err := c.HRIS.Benefits.Update(
		context.TODO(),
		"string",
		finchgo.HRISBenefitUpdateParams{Description: finchgo.F("string")},
	)
	if err != nil {
		var apierr *finchgo.Error
		if errors.As(err, &apierr) {
